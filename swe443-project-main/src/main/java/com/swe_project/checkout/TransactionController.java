...
import com.swe_project.history.HistoryService;
...
public class TransactionController {
    ...
    public String AddToCart(@RequestParam(name = "bookID") long bookID, Model model) {
        CheckoutBook myBook = cart.getBookAvailable(bookID);
        if(myBook!=null)
    ...
    public String completeTransaction(Model model) {
        ...
        for(CheckoutBook b: cart.getBooksSelected()){
            bookIDs.add(b.getId());
        }
...
@GetMapping("/checkout/update")
    ...
    private List<CheckoutBook> GetBookDetails(){//REST CALL 2
        RestTemplate restTemplate = new RestTemplate();
        ResponseEntity<List<CheckoutBook>> libraryResponse;
        List<CheckoutBook> list = new ArrayList<>();
        try {
            libraryResponse = restTemplate.exchange("http://localhost:8080/library/books", HttpMethod.GET, null, new ParameterizedTypeReference<List<CheckoutBook>>(){});
            list = libraryResponse.getBody();
        }catch(Exception e) {

            for (long i = 0; i < 5; i++) {
                list.add(new CheckoutBook(i,"","Name "+i,0.0+i,0,0,0));
            }
        }
        return list;
    }
